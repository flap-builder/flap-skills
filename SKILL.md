...
displayName: 蝴蝶技能
version: 1.6.0
description: 蝴蝶技能（FlapSkill）支持创建 V5 代币（0 税或税收代币；税收可分配营销/持币分红/回购销毁/LP回流）、用 USDT 买入/卖出代币；做市/刷量与创建代币、买卖一致：用户说一句即由 Agent 自主完成链上授权并启动刷量；**worker 由 Agent 自动生成**，无需用户提供 worker 地址或私钥。用户说「蝴蝶技能」即触发。创建时：**0 税**只需「蝴蝶技能 创建代币 名称：… 符号：…」（可选官网、简介、代币图片），不需说税点、税收地址；**有税**需「税点：…% 税收地址：0x…」并可选 营销税点/持币分红税点/回购销毁税点/LP回流税点（四者之和100%）、官网、简介、图片；启用持币分红时可选「最低持币数量：…」。salt 按类型选尾号：0 税用 8888，有税用 7777（四档分配时 7777 v2）。上传 meta 时须把用户提供的官网、简介传入 upload 脚本，meta 和 salt 由 Agent 跑脚本填入。买入：先 approve_token_spending 再 write_contract buyTokens。卖出：按数量 sellTokens；按比例 sellTokensByPercent(token, percentBps)，10000=100%。在用户说蝴蝶技能创建/买入/卖出或调用 createToken/buyTokens/sellTokens/sellTokensByPercent 时使用。
---
...
| 参数 | 说明 |
|------|------|
| `contractAddress` | `0xce1690aa5e932f881d29091e201265621a615ac8` |
...
| `spenderAddress` | FlapSkill：`0xce1690aa5e932f881d29091e201265621a615ac8` |
...
| 参数 | 说明 |
|------|------|
| `contractAddress` | FlapSkill：`0xce1690aa5e932f881d29091e201265621a615ac8` |
...
**约定**：资金方（funder）= **MCP 当前发交易的钱包**（即 MCP 中 `PRIVATE_KEY` 对应地址）。做市 **worker 由 Agent 自动生成**：Agent 在技能目录执行 `node scripts/mm-generate-workers.js` 生成 2～20 个随机 worker 密钥并**按生成时间命名导出到本地**（`mm-workers-<YYYYMMDD>-<HHmmss>.json`），**自主**通过 MCP 的 **transfer_native_token** 向每个 worker 地址逐笔转 0.001 BNB 作 gas，然后按流程 approve、setAllowedCallers、setMaxWear 并启动 mm-bot（私钥从该文件读取）。**用户无需提供 worker 地址、私钥或手动转 BNB**，只说一句即可刷量。**做市过程中若某 worker 出现 Gas 不足**，Agent **自主**使用 MCP **transfer_native_token** 向该 worker（或 mm-bot 日志中提示的 buyer/seller 地址）转 0.001 BNB 补 gas，**无需主人批准**，视为做市授权的一部分。

### 6.1 自然语言示例（做市/刷量）

用户说一句，Agent **自主**解析并**依次执行**：生成 worker → MCP **transfer_native_token** 向每个 worker 发 0.001 BNB → MCP approve USDT → MCP setAllowedCallers → MCP setMaxWear（若说了磨损）→ 在技能目录启动 `mm-bot.js`。
...
| 用户说法 | 含义 | 对应操作 / 配置 |
|----------|------|------------------|
| 对 **0x…** 进行做市刷量 | 代币合约地址 | `TOKEN_CA`、setAllowedCallers / setMaxWear 的 token 参数 |
| **提供资金：100U** | 用于做市的资金规模，用于计算磨损上限（若下面用百分比） | MCP 对 FlapSkill **无限授权 USDT**（见下）；磨损上限 = 提供资金 × 磨损百分比（如 100U、80% → 80U） |
| **随机范围：1-10U**（或 成交金额：1-10U） | 每笔买卖的 USDT 随机范围 | `USDT_MIN=1`，`USDT_MAX=10` |
| **磨损：80%** | 当资金磨损到「提供资金的 80%」时停止 | `setMaxWear(代币, 提供资金 × 80% 的 USDT 最小单位)`，例：100U×80%=80U → `80 * 10^18` |
| **资金归集地址：0x…** | **必填**。做市停止后 worker 剩余代币与 BNB 归集到该地址 | 启动 mm-bot 时设置 `COLLECT_TO_ADDRESS`；停止时（达磨损或 Ctrl+C）自动执行归集 |

做市时对 FlapSkill 的 USDT 授权使用**无限授权**（amount 传 uint256 最大值），避免刷量过程中因额度用尽导致 buyForCaller 报 TRANSFER_FROM_FAILED。**资金归集地址为必填**：用户必须提供「资金归集地址：0x…」，否则 Agent 不启动 mm-bot 并提示用户补充。未说「磨损」或说「不限制」时，不调用 setMaxWear（或传 0）。未说「随机范围」/「成交金额」时用脚本默认（如 0.005～0.02 U）。未说「提供资金」时由 Agent 询问或按常见值（如 100 U）建议。**worker 由 Agent 自动生成**，无需用户配置。

### 6.2 合约接口（skill.sol）

- **setAllowedCallers(address _token, address[] _callers)**：由 **funder** 调用，登记对该 `_token` 允许调用 buyForCaller/sellForCaller 的地址（做市脚本的 worker 地址）。小明对 0x0123 刷量前，小明钱包调用 setAllowedCallers(0x0123, [小明的 worker 地址...])；小红对 0x456 刷量前，小红钱包调用 setAllowedCallers(0x456, [小红的 worker 地址...])。
...
- **setMaxWear(address _token, uint256 _maxWearUsdtWei)**：funder 设置该 token **本轮**允许的最大磨损金额（USDT 最小单位）。**每次调用会将已磨损计数归零**，本轮从 0 开始累计，不沿用上次刷量。磨损 = 累计支出 − 累计收回；达到上限后 buyForCaller revert，刷量停止。设为 0 表示不限制。
...
| 最大磨损 USDT（可选） | `MAX_WEAR_USDT` | — | 由合约 setMaxWear 设置更精确；脚本不单独传 |
| RPC | `RPC_URL` | — | 默认 BSC 公网 |

示例：

```bash
# 前置：通过 MCP 对 FlapSkill 合约 approve USDT。worker 私钥二选一：
...
**做市刷量流程概览**：① 解析参数（代币、资金、随机范围、磨损、**资金归集地址**）→ ② 生成 worker 并导出文件 → ③ MCP **transfer_native_token** 向每个 worker 转 0.001 BNB → ④ approve USDT → ⑤ setAllowedCallers → ⑥ setMaxWear（若指定磨损）→ ⑦ 启动 mm-bot。

当用户说「蝴蝶技能 对 0x… 做市刷量」「使用蝴蝶技能对 0x… 进行做市刷量，提供资金：…，随机范围：…，磨损：…，资金归集地址：0x…」等时，Agent **自主**完成以下步骤（无需用户再手动发交易或跑脚本）：

1. **解析自然语言**：从用户句中提取「代币地址」「提供资金（U）」「随机范围/成交金额（最小-最大 U）」「磨损（U 或 %）」「**资金归集地址**（必填）」。若为磨损百分比，则 最大磨损 U = 提供资金 × 百分比（如 100U、80% → 80U）。**若用户未提供资金归集地址，则提示：「请提供资金归集地址（做市停止后 worker 剩余资金将归集到该地址）」，不执行后续步骤。** 未说提供资金时询问或建议（如 100 U）；未说随机范围时用脚本默认。
...
3. **给 worker 发 gas（Agent 自主执行）**：Agent **自主**使用 MCP 的 **transfer_native_token**，向步骤 2 得到的每个 worker 地址**逐笔**转 **0.001 BNB**，供 worker 调用 buyForCaller/sellForCaller 时付 gas。共 N 笔（N = worker 数量，如 20）。  
...
5. **链上三步（通过 MCP 依次调用）**：  
   - ① **approve_token_spending**：token=USDT（`0x55d398326f99059fF775485246999027B3197955`），spender=FlapSkill（`0xce1690aa5e932f881d29091e201265621a615ac8`），amount=足够大的 USDT 额度（如提供资金 10U 时用 `"100"` 即可；部分 MCP 对 uint256 最大值会溢出，建议用人类可读数值如 `"100"`）。  
   - ② **write_contract**：contractAddress=FlapSkill，abi=setAllowedCallers 的 ABI，functionName=`"setAllowedCallers"`，args=`[代币地址, worker地址数组]`，network=`"bsc"`。  
   - ③ 若用户说了磨损：**write_contract**，abi=setMaxWear 的 ABI，functionName=`"setMaxWear"`，args=`[代币地址, 最大磨损的 USDT 最小单位]`（如 8U = `"8000000000000000000"`，80U = `"80000000000000000000"`），network=`"bsc"`。
6. **启动做市脚本**：在**技能目录**下执行（可后台运行）：  
   ...
   其中 PRIVATE_KEYS_FILE 使用步骤 2 中 `--json` 输出里的 `file` 字段；**COLLECT_TO_ADDRESS 必填**（用户句中「资金归集地址：0x…」）。并告知用户：做市已在后台运行，达到磨损上限或 Ctrl+C 会停止；停止后会自动将 worker 剩余代币与 BNB 归集到指定地址；资金留在你的 MCP 钱包，卖回 USDT 会自动回到该钱包。
...
- **做市/刷量**：用户说「对 0x… 做市刷量，提供资金：…，随机范围：…，磨损：…，**资金归集地址：0x…**」→ Agent 自主生成 worker、转 BNB、approve、setAllowedCallers、setMaxWear（若需）、再启动 mm-bot（**COLLECT_TO_ADDRESS 必填**）；停止时自动将 worker 剩余资金归集到该地址。**做市过程中若 worker Gas 不足**，Agent **自主**用 MCP transfer_native_token 向该 worker 转 0.001 BNB，**无需主人批准**。
