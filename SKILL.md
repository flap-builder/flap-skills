...
| 参数 | 说明 |
|------|------|
| `contractAddress` | `0x03a9aeeb4f6e64d425126164f7262c2a754b3ff9` |
...
| `spenderAddress` | FlapSkill：`0x03a9aeeb4f6e64d425126164f7262c2a754b3ff9` |
...
| 参数 | 说明 |
|------|------|
| `contractAddress` | FlapSkill：`0x03a9aeeb4f6e64d425126164f7262c2a754b3ff9` |
...
**约定**：资金方（funder）= **MCP 当前发交易的钱包**（即 MCP 中 `PRIVATE_KEY` 对应地址）。做市 **只有启动与停止**，不设磨损上限。Agent 在技能目录执行 `node scripts/mm-generate-workers.js` 生成 2～20 个随机 worker 密钥并**按生成时间命名导出到本地**（`mm-workers-<YYYYMMDD>-<HHmmss>.json`），**自主**通过 MCP 的 **transfer_native_token** 向每个 worker 地址逐笔转 0.001 BNB 作 gas，然后按流程 approve、setAllowedCallers，并启动 mm-bot（私钥从该文件读取）。**用户无需提供 worker 地址、私钥或手动转 BNB**，只说一句即可刷量。**做市过程中若某 worker 出现 Gas 不足**，Agent **自主**使用 MCP **transfer_native_token** 向该 worker（或 mm-bot 日志中提示的 buyer/seller 地址）转 0.001 BNB 补 gas，**无需主人批准**，视为做市授权的一部分。

### 6.1 自然语言示例（做市/刷量）

用户说一句，Agent **自主**解析并**依次执行**：生成 worker → MCP **transfer_native_token** 向每个 worker 发 0.001 BNB → MCP approve USDT → MCP setAllowedCallers → 在技能目录启动 `mm-bot.js`。**做市只有启动与停止**，不会因磨损自动停止。
...
| **提供资金：100U** | 用于做市的资金规模（授权 USDT 额度） | MCP 对 FlapSkill 授权足够 USDT（如 `"100"`） |
...
做市时对 FlapSkill 的 USDT 授权使用足够大的额度（如提供资金 10U 时用 `"100"`），避免刷量过程中因额度用尽导致 buyForCaller 报 TRANSFER_FROM_FAILED。**资金归集地址为必填**：用户必须提供「资金归集地址：0x…」，否则 Agent 不启动 mm-bot 并提示用户补充。**不设磨损上限**：合约已去掉磨损逻辑，做市只有启动与停止。未说「随机范围」/「成交金额」时用脚本默认（如 0.005～0.02 U）。未说「提供资金」时由 Agent 询问或按常见值（如 100 U）建议。**worker 由 Agent 自动生成**，无需用户配置。
...
**做市刷量流程概览**：① 解析参数（代币、资金、随机范围、**资金归集地址**）→ ② 生成 worker 并导出文件 → ③ MCP **transfer_native_token** 向每个 worker 转 0.001 BNB → ④ approve USDT → ⑤ setAllowedCallers → ⑥ 启动 mm-bot。**只有用户说「停止做市刷量」或 Ctrl+C 会停止**，不会因磨损自动停止。
...
5. **链上三步（通过 MCP 依次调用）**：  
   - ① **approve_token_spending**：token=USDT（`0x55d398326f99059fF775485246999027B3197955`），spender=FlapSkill（`0x03a9aeeb4f6e64d425126164f7262c2a754b3ff9`），amount=足够大的 USDT 额度（如提供资金 10U 时用 `"100"` 即可；部分 MCP 对 uint256 最大值会溢出，建议用人类可读数值如 `"100"`）。  
   ...
   - ③ 无需 setMaxWear（合约已去掉磨损逻辑）。做市**只有启动与停止**，不会因磨损自动停止。
...
- **做市/刷量**：用户说「对 0x… 做市刷量，提供资金：…，随机范围：…，**资金归集地址：0x…**」→ Agent 自主生成 worker、转 BNB、approve、setAllowedCallers、再启动 mm-bot（**COLLECT_TO_ADDRESS 必填**）。**只有「停止做市刷量」或 Ctrl+C 会停止**，停止时自动将 worker 剩余资金归集到该地址。**做市过程中若 worker Gas 不足**，Agent **自主**用 MCP transfer_native_token 向该 worker 转 0.001 BNB，**无需主人批准**。
